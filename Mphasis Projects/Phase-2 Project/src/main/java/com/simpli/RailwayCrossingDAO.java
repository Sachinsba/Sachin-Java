...
public class RailwayCrossingDAO {
	...
	public void deleteCrossing(int crossingId) {
		try {
			...
			try (PreparedStatement deleteFavoriteCrossingsStatement = connection.prepareStatement(deleteFavoriteCrossingsQuery)) {
	...
	public List<RailwayCrossing> getFavoriteCrossings() {
		...
		try (Connection connection = DatabaseConnection.getConnection()) {
			String query = "SELECT rc.* FROM railway_crossing rc "+ "JOIN favorite_crossing fc ON rc.id = fc.railway_crossing_id";
			PreparedStatement statement = connection.prepareStatement(query);
			ResultSet resultSet = statement.executeQuery();
			while (resultSet.next()) {
				RailwayCrossing crossing = new RailwayCrossing();
